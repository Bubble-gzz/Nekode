...
public class Puzzle3_1 : PuzzleLogic
{
    ...
    override protected IEnumerator GameProcess()
    {
        ...
        DrBubble.instance.transform.position = new Vector3(30.8f, 31.5f, 0);
        yield return new WaitForSeconds(1f);
        dialogue.SetTailLM();
        dialogue.Open();
        
        if (Settings.language == "CH") dialogue.Play("每当出现一条直线，就代表会有新鲜事物。");
        else dialogue.Play("Whenever a straight line appears, something new will be introduced.", new Vector2(680, 160));
        while (dialogue.isPlaying) yield return null;

        if (Settings.language == "CH") dialogue.Play("我称之为直线定律。");
        else dialogue.Play("I called it Law of Straight Line.", new Vector2(630, 120));
        while (dialogue.isPlaying) yield return null;

        if (Settings.language == "CH") dialogue.Play("没错，这次我们又为你准备了新道具。");
        else dialogue.Play("And of course we've got another new stuff for you this time.", new Vector2(630, 160));
        while (dialogue.isPlaying) yield return null;

        if (Settings.language == "CH") dialogue.Play("看到这三个透明的加法方块了吗？");
        else dialogue.Play("Look at these 3 transparent add tiles,", new Vector2(780, 120));
        while (dialogue.isPlaying) yield return null;

        if (Settings.language == "CH") dialogue.Play("透明意味着你没有权限编辑。");
        else dialogue.Play("Transparency means that you don't have permission to edit them.", new Vector2(780, 160));
        while (dialogue.isPlaying) yield return null;

        if (Settings.language == "CH") dialogue.Play("而我希望你将小猫的数字输出到端口A！");
        else dialogue.Play("But I want you to output the kitten's number to port A!", new Vector2(740, 160));
        while (dialogue.isPlaying) yield return null;

        if (Settings.language == "CH") dialogue.Play("为了完成这个不可能的任务，你需要了解一下IO的概念。");
        else dialogue.Play("To accomplish this impossible task, you need to learn about I/O.", new Vector2(740, 160));
        while (dialogue.isPlaying) yield return null;
